...
| `agent4j-core` tool abstractions | `packages/agent/src/types.ts` | Started | Basic registry/executor exists with sequential/parallel execution modes. Need PI-style update callbacks, before/after tool hooks, blocked tool results, and terminate hints. |
