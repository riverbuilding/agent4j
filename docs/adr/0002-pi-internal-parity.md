...
| SDK/runtime API | `packages/coding-agent/src/core/agent-session*.ts`, `sdk.ts` | Phase 9 shape audit complete | Must mirror `AgentSession`, `AgentSessionRuntime`, services, new/resume/fork/clone/import responsibilities after Phase 8 closes the conversation ownership gap. Java target ownership is pinned in `docs/sdk-runtime-shape-audit.md`: `agent4j-core` stays generic, `agent4j-coding` owns the coding SDK/runtime, runtime/session APIs own canonical conversation context, and auth/login is a runtime service before CLI wrappers. Exact PI source names/lifecycle details still need final source validation before Phase 9 closeout. |
