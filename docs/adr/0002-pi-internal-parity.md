...
| `agent4j-core` tool abstractions | `packages/agent/src/types.ts` | Started | Basic registry/executor exists with sequential/parallel execution modes, `ToolContext` update callbacks, loop-level before/after tool hooks, and hook-returned blocked tool results. Need PI-style terminate hints. |
