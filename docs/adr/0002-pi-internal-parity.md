...
| Current AI/core bridge | `agent4j-core.AgentMessage` still stores JSON content while `agent4j-ai` has typed content | Transitional state from early Phase 4; kept temporarily to preserve PI session JSONL round-tripping. | Follow the AgentMessage migration plan in `docs/implementation-plan.md`: add typed transcript views, migrate converters and loop internals to views, then decide whether to promote the view hierarchy into the primary Java transcript model. |
