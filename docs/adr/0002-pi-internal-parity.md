...
| `agent4j-core` agent loop | `packages/agent/src/agent-loop.ts` | Started | Current loop supports fake text/tool-call turns, emits PI-style start/update/end event boundaries, emits prompt user `message_start`/`message_end` events before the assistant stream, and appends tool-result transcript messages in execution order. `turn_end` intentionally repeats the round assistant message and ordered tool-result messages, matching PI summary-event behavior; `agent_end` returns all new messages for the invocation. Needs queue drain points, retry behavior, abort behavior, and parallel/ordered tool-result semantics. |
...
| Session JSONL model | `packages/coding-agent/src/core/session-manager.ts`, `packages/agent/src/harness/session/*` | In progress | Existing codec/manager cover core entries. Need parent validation, resume/fork stale snapshot semantics, and parity fixtures. |
