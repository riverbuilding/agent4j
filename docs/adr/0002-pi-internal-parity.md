...
| Session JSONL model | `packages/coding-agent/src/core/session-manager.ts`, `packages/agent/src/harness/session/*` | In progress, Phase 8 fix target | Existing codec/manager cover core entries, can append Phase 4+ `AgentMessage` loop outputs as PI-shaped message entries, append full `AgentLoopResult.messages()` for repeated runs, rebuild active-path message entries as `AgentMessage` envelopes for resume, reject stale same-file append snapshots with a reopen-required error, validate append-log parent integrity for resume/import, persist compaction summaries/retained tail entries, and append branch summaries. Need malformed typed payload validation and broader parity fixtures. |
