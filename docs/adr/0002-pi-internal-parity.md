...
| `agent4j-core` tool abstractions | `packages/agent/src/types.ts` | Started | Basic registry/executor exists with sequential/parallel execution modes, `ToolContext` update callbacks, and loop-level before/after tool hooks. Need PI-style blocked tool results and terminate hints. |
