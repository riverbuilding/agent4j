...
| SDK/runtime API | `packages/coding-agent/src/core/agent-session*.ts`, `sdk.ts` | Phase 9 core interface baseline complete | Must mirror `AgentSession`, `AgentSessionRuntime`, services, new/resume/fork/clone/import responsibilities after Phase 8 closes the conversation ownership gap. Java target ownership is pinned in `docs/sdk-runtime-shape-audit.md`: `agent4j-core` stays generic, `agent4j-coding` owns the coding SDK/runtime, runtime/session APIs own canonical conversation context, and auth/login is a runtime service before CLI wrappers. Java now has initial SDK interfaces and request/response records under `com.agent4j.coding.sdk` for session lifecycle, prompt calls, prompt results, and SDK event subscription. Concrete runtime behavior and exact PI source names/lifecycle details still need later Phase 9 slices. |
