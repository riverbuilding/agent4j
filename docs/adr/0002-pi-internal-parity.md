...
| Session JSONL model | `packages/coding-agent/src/core/session-manager.ts`, `packages/agent/src/harness/session/*` | Complete for Phase 8 validation slices, fixture expansion pending | Existing codec/manager cover core entries, can append Phase 4+ `AgentMessage` loop outputs as PI-shaped message entries, append full `AgentLoopResult.messages()` for repeated runs as one freshness-checked locked batch, rebuild active-path message entries as `AgentMessage` envelopes for resume, reject stale same-file append snapshots with a reopen-required error, validate append-log parent integrity for resume/import, validate known typed payloads before resume/import/append, persist compaction summaries/retained tail entries, and append branch summaries. Need broader parity fixtures. |
